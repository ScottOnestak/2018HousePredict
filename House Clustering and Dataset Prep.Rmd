...
```{r House Results Agg,cache=TRUE}
...
```{r plots and plots and plots,cache=TRUE}
...
```{r plots and plots and plots of clusters,cache=TRUE}
...
```{r export cluster data,cache=TRUE}
clusters = norm[,c(3,24)]
write.csv(clusters,file="clusters.csv",row.names = F)
```

```{r cluster analysis - density plots,cache=TRUE}
...
```{r write demographics}
write.csv(CDs,file="CDs.csv",row.names = F)
```

```{r get closest CDs list}
