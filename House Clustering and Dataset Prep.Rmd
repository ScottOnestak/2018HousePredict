...
CDs = Demographics %>%
  mutate(VotingAge=rowSums(Demographics[,c("Pop18to24","Pop25to35","Pop35to45","Pop45to55","Pop55to65","Pop65to75","Pop75Over")],na.rm=T),
         ...
         MedianEarnings_HSBach_Diff = MedianEarnings_Bach - MedianEarnings_HS) %>%
  dplyr::select(State,Congressional.District.Number,CD_Name,TotalPop,VotingAge,Under35,Over65,MedianAge,Male,RaceBase,
...
CDs = CDs %>%
  ...
  dplyr::select(-VotingAge,-TotalPop,-RaceBase,-MaritalBase,-EducationBase,-PovertyBase)
```
...
```{r normalize}
CDs_sub = CDs[,-c(4,5,10,12,14,15,16,20,22,24)]
norm = cbind(CDs_sub[,c(1:3)],scale(CDs_sub[,c(4:16)]))

plot(norm[,c(4:10,16)],pch=20)
plot(norm[,c(11:16)],pch=20)
```

```{r pca}
pcs = princomp(norm[,c(4:16)])
plot(pcs)
plot(pcs, type='l')
summary(pcs)

pcs2 = prcomp(norm[c(4:16)])
comp <- data.frame(pcs2$x[,1:5])
plot(comp, pch=36)
```
...
#choose 6
kmeans = kmeans(comp,centers = 6,iter.max = 1000,nstart=25)
norm$cluster = kmeans$cluster
```

```{r plots and plots and plots of clusters}
palette(c("black","cyan","chartreuse4","red","magenta","grey"))
plot(norm[,c(4:9,16)],col=norm$cluster,pch=16)
plot(norm[,c(10:15,16)],col=norm$cluster,pch=16)
```

```{r export cluster data}
clusters = norm[,c(3,29)]
