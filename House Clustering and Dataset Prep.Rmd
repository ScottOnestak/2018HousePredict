...
norm = cbind(CDs_sub[,c(1:3)],scale(CDs_sub[,c(4:19)]))

plot(norm[,c(4:11,18)],pch=20)
plot(norm[,c(12:19)],pch=20)
```

```{r pca}
pcs = princomp(norm[,c(4:19)])
plot(pcs)
plot(pcs, type='l')
summary(pcs)

pcs2 = prcomp(norm[c(4:19)])
...
plot(norm[,c(4:11,18)],col=norm$cluster,pch=16)
plot(norm[,c(12:19)],col=norm$cluster,pch=16)
```

```{r export cluster data}
clusters = norm[,c(3,20)]
