...
CDs = Demographics %>%
  mutate(VotingAge=rowSums(Demographics[,c("Pop18to24","Pop25to35","Pop35to45","Pop45to55","Pop55to65","Pop65to75","Pop75Over")],na.rm=T),
         ...
         MedianEarnings_HSBach_Diff = MedianEarnings_Bach - MedianEarnings_HS) %>%
  dplyr::select(State,Congressional.District.Number,CD_Name,TotalPop,VotingAge,Under35,Over65,MedianAge,Male,RaceBase,Hispanic,Black,
...
```{r normalize}
norm = cbind(CDs[,c(1:3)],scale(CDs[,c(4:28)]))

plot(norm[,c(4:15,28)],pch=20)
plot(norm[,c(16:28)],pch=20)
```

```{r pca}
pcs = princomp(norm[,c(4:28)])
plot(pcs)
plot(pcs, type='l')
summary(pcs)

pcs2 = prcomp(norm[c(4:28)])
comp <- data.frame(pcs2$x[,1:6])
plot(comp, pch=36)
```
...
plot(norm[,c(4:9,28)],col=norm$cluster,pch=16)
plot(norm[,c(10:13,28)],col=norm$cluster,pch=16)
plot(norm[,c(14:19,28)],col=norm$cluster,pch=16)
```
