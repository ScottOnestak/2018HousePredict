...
theCDsToModel$GOP_PrctAdj = theCDsToModel$GOP_Prct + .5*theCDsToModel$ActAdjPVI_chg
theCDsToModel$Dem_PrctAdj = theCDsToModel$Dem_Prct - .5*theCDsToModel$ActAdjPVI_chg
...
#REDUCE FEATURS FOR MODEL
```{r feature selection}
featureTest = modeldata[,c(6:26,28,29,31:126,128:133,138:140)]

y = "GOP_Result"
x = setdiff(names(featureTest),c(y,"Dem_Result"))

h2o.init(nthreads=-1,max_mem_size='6G')

featureTest.h2o = as.h2o(featureTest)

rf_features_GOP = h2o.randomForest(x = x,
                      y = y,
                      training_frame = featureTest.h2o,
                      ntrees = 250,
                      nfolds = 10,
...
h2o.varimp(rf_features_GOP)[1:20,]
h2o.varimp(rf_features_GOP)[21:40,]
h2o.varimp_plot(rf_features_GOP)

y = "Dem_Result"
x = setdiff(names(featureTest),c(y,"GOP_Result"))

rf_features_Dem = h2o.randomForest(x = x,
                      y = y,
                      training_frame = featureTest.h2o,
                      ntrees = 250,
                      nfolds = 10,
...
featureTest.h2o$Gap = featureTest.h2o$GOP_Result - featureTest.h2o$Dem_Result

y = "Gap"
x = setdiff(names(featureTest),c(y,"GOP_Result","Dem_Result"))

rf_features_Gap = h2o.randomForest(x = x,
                      y = y,
                      training_frame = featureTest.h2o,
                      ntrees = 250,
                      nfolds = 10,
...
train_set = modeldata[,c(1:5,8,14,28,31,33:35,38,40,41,43,48,54,56,58,59,60,63,65,66,68,73,79,81,83:88,92,93,96,97,99,100,102,113,121:123,127:131,133,138:140)]
test_set = currTest[,c(1:5,8,14,28,31,33:35,38,40,41,43,48,54,56,58,59,60,63,65,66,68,73,79,81,83:88,92,93,96,97,99,100,102,113,121:123,127:131,133,138:140)]
...
y = "winner"
x = setdiff(names(train),c(y,"name","State","District","CD_Name","year"))
...
ensemble = h2o.stackedEnsemble(x = x,
                                y = y,
                                training_frame = train,
                                model_id = "scott_ensemble",
                                base_models = list(rf,))


test_results = h2o.predict(ensemble, newdata = test)
test_resultsrf = h2o.predict(rf,newdata = test)
test_resultsgb = h2o.predict(gb,newdata = test)
test_resultsnb = h2o.predict(nb,newdata = test)
test_set$results = as.data.frame(test_results$p1)
test_set$resultsrf = as.data.frame(test_resultsrf$p1)
test_set$resultsgb = as.data.frame(test_resultsgb$p1)
see_results = test_set[,c(1,56,57,58)]
